...
class MapFieldLayer extends PositionComponent {
  ...
  late FieldPiece xxfie;
  late List<List<FieldPiece>> pieceList = List.generate(
      kFieldColumnLimit,
      (_) => List.generate(kFieldRowLimit, (_) {
            Future<FieldPiece> futurefie = FieldPiece.create('null');
            futurefie.then((result) => xxfie = result);
            return xxfie;
          }));
...
class FieldPiece extends PositionComponent {
  ...
  int column;
  int row;
  late String mode; //生成後初回移動か否かを判定するフラグ。map画面のtempListからpieceをremoveするために使用。
  ...
  static Future<FieldPiece> create(fieldType,
      {initX = 0, initY = 0, mode = "button", column = 0, row = 0}) async {
    FieldPiece fiepie = FieldPiece(fieldType,
        initX: initX, initY: initY, mode: mode, column: column, row: row);
    await fiepie.initFieldPiece();
    return fiepie;
  }

  Future<void> initFieldPiece() async {
    //image sprite
...
class FieldButtonHEX extends PolygonComponent
  ...
  void onTapUp(TapUpEvent event) {
    ...
    if (xxidx < 10) {
      ...
      Future<FieldPiece> futurePiece = FieldPiece.create(fieldButton.fieldType,
          initX: xxidx * 5, initY: xxidx * 5, mode: "temp");
      futurePiece.then((xxpiece) {
